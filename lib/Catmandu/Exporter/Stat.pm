...
sub commit {
    ...
    for my $key (@keys) {
        ...
        $stats->{count}    = List::Util::sum0(@$val);
        $stats->{min}      = List::Util::min(@$val);
        $stats->{max}      = List::Util::max(@$val);
        $stats->{mean}     = Statistics::Basic::mean($val);
        $stats->{median}   = Statistics::Basic::median($val);
        $stats->{mode}     = Statistics::Basic::mode($val);
        $stats->{variance} = Statistics::Basic::variance($val);
        $stats->{stdev}    = Statistics::Basic::stddev($val);
        $stats->{zeros}    = int(grep {$_ == 0} @$val);
        $stats->{'zeros%'} = sprintf "%.1f" , @$val > 0 ? 100 * $stats->{zeros} / int(@$val) : 100;
